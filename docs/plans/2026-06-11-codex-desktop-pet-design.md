...
4. 非 `codex_running` 状态必须有独立姿态族；状态语义通过局部姿态、节奏、气泡和轻量叠加装饰共同表达，避免低质量旧动作造成整图晃动感。
...
v9 决策：用户确认 `codex-running` 的梦幻跳舞效果整体最协调，但“所有状态都像同一个跳舞动作”仍然不够清楚。因此当前方案以 `codex-running` 作为角色源帧和运行态主循环，其它状态由 `scripts/generate-state-action-frames.py` 生成独立动作目录。生成过程对头部、上半身、手臂、裙摆做局部区域姿态变形，保留同一位梦幻白色半透明少女，同时让状态动作差异更明显。
...
验收时重点看三件事：人物动作必须协调、像人物自己在动；不同状态的轮廓和姿态必须一眼能区分；整张人物图的全局位移和旋转必须保持很小，避免回到“图片整体漂浮”的观感。
...
4. `codex_running` 使用当前梦幻跳舞动作；其它状态使用从该源帧生成的独立局部姿态 clip，不再出现小键盘或低协调旧动作。
5. 舞台地面必须由 AppKit 动态绘制：水面光带、淡倒影、动态裙摆光点。
6. 8 个状态保留动作目录和元数据，原生人物 clip 按状态独立加载；状态差异由局部姿态、装饰层、气泡和播放频率共同完成。
...
- 角色方向：梦幻白色半透明少女。
- 动作方案：`codex-running` 加载协调跳舞主循环，其它状态加载 `public/assets/dancer-actions/<state>/frame-*.png` 下的独立局部姿态 clip，状态装饰由 AppKit 动态绘制。
...
- 所有非 `codex-running` clip 都从协调跳舞源帧派生，并使用局部区域姿态变形来保持人物美观和动作连续。
- 当前测试要求：原生映射必须按状态返回独立 action id；非 `codex-running` clip 的 `action.json` 必须声明 `motionTechnique = local-region-pose-transform`；全局位移和旋转仍必须保持低幅度。
