...
本版本作为当前开发实现规格，目标是解决三项明确问题：桌宠尺寸不够大、底部舞台和裙摆光点不明显、状态动作没有真正对应 Codex 状态。v4 进一步修正“多个状态看起来都在跳舞”的问题：除 `codex_running` 外，其它状态必须使用不同关键姿态，而不是共享舞蹈帧做轻微变形。
...
2. 当前 8 帧梦幻少女动效方向比矢量拼接人形更美观，但不能继续作为所有状态的唯一动作。
3. 桌宠当前面积偏小，默认角色高度必须接近或超过常见 1080p 屏幕高度的一半。
4. `codex_running` 状态保留当前跳舞动作；其它状态必须有独立关键姿态，肉眼轮廓要明显不同。
...
| --- | --- | --- | --- | --- |
| `idle` | 未检测到 Codex 或 Codex 空闲 | 坐着发呆、眨眼、慢呼吸 | 安静、低频、像在等你回来 | 无 |
| `codex_running` | 检测到 Codex 正在运行 | 当前梦幻少女跳舞动作 | 温和工作感，裙摆轻摆，不显示小键盘 | 无 |
| `command_running` | 正在执行命令或测试 | 盯着终端，头发或配饰发光 | 专注、比普通运行更紧张 | 无 |
...
| `success` | 任务成功、测试通过 | 跳一下、转半圈、星光效果 | 短暂庆祝 | `通过啦` |
| `error` | 命令失败、测试失败、异常 | 歪头、冒汗、拿放大镜看日志 | 有问题但不吓人 | `这里好像出错了` |
...
| 状态 | 播放方式 | 建议帧间隔 | 循环策略 |
| --- | --- | --- | --- |
| `idle` | 静态主姿态加偶发眨眼 | `1.20s` | 低功耗慢循环，舞台不持续重绘 |
| `codex_running` | 跳舞循环 | `0.42s` | 连续循环，动作幅度中等 |
| `command_running` | 终端专注循环 | `0.38s` | 连续循环，发光增强 |
...
| `success` | 跳跃、转圈 | `0.30s` | 播放 `1` 到 `2` 次后收敛 |
| `error` | 歪头、冒汗、看日志 | `0.72s` | 播放后停在困惑姿态 |
...
v7 决策：用户更喜欢 `codex-running` 的梦幻跳舞效果，因此持续时间较长、不会强打断的状态复用这套跳舞基底，再用节奏、气泡和轻量叠加装饰表达状态语义。短暂或强语义状态仍使用独立动作。

- `idle`：坐姿或跪坐休息，低频呼吸、轻微回落。
- `codex-running`：沿用当前梦幻跳舞动作，不再绘制小键盘。
- `command-running`：前倾专注，面向半透明终端光屏。
- `thinking`：复用梦幻跳舞基底，慢速播放，叠加思考光点。
- `long-running`：复用梦幻跳舞基底，极慢速播放，叠加咖啡/疲惫提示。
- `waiting-user`：复用梦幻跳舞基底，慢速播放，叠加挥手提醒和气泡。
- `success`：跳起庆祝，配合星光。
- `error`：歪头困惑，拿放大镜或日志观察。

验收时重点看两件事：长时间停留的状态要漂亮、耐看；强语义状态要一眼能看懂。
...
4. `codex_running` 使用当前梦幻跳舞动作，不再出现小键盘。
5. 舞台地面必须由 AppKit 动态绘制：水面光带、淡倒影、动态裙摆光点。
6. 8 个状态必须各有独立动作目录并由原生播放器按状态切换。
...
- 角色方向：梦幻白色半透明少女。
- 动作方案：按 Codex 状态加载 `public/assets/dancer-actions/<clip>/frame-*.png`。
- 状态来源：进程检测 + 本地 `state.json`。
- 第一阶段重点：把“足够大、舞台梦幻、光点动态、状态动作真实切换且动作语义明显不同”做好。
