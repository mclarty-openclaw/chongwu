...
本版本作为当前开发实现规格，目标是解决三项明确问题：桌宠尺寸不够大、底部舞台和裙摆光点不明显、状态动作没有真正对应 Codex 状态。v9 根据最新视觉反馈调整：`codex-running` 保留当前最协调的梦幻跳舞帧，作为角色风格和身体比例的源帧；其它状态不再统一复用同一个跳舞 clip，而是从这套源帧生成独立的本地姿态动作，让前倾、思考、疲惫、挥手、庆祝、报错等状态一眼能分辨。
...
4. 非 `codex_running` 状态必须有独立姿态族；状态语义通过完整源姿态序列、节奏、气泡和轻量叠加装饰共同表达，避免局部拼接造成残影，也避免低质量旧动作造成整图晃动感。
...
| --- | --- | --- | --- | --- |
| `idle` | 未检测到 Codex 或 Codex 空闲 | 站姿呼吸、极慢轻摆 | 安静、低频、像在等你回来 | 无 |
| `codex_running` | 检测到 Codex 正在运行 | 当前梦幻少女跳舞动作 | 温和工作感，裙摆轻摆，不显示小键盘 | 无 |
| `command_running` | 正在执行命令或测试 | 前倾专注，叠加终端光屏 | 专注、比普通运行更紧张 | 无 |
| `thinking` | 正在分析、生成、阅读上下文 | 侧身托腮/转头思考，叠加思考光点 | 思考感，动作慢，不打断视线 | 无 |
| `long_running` | 单个任务运行超过阈值 | 低头疲惫、慢速轻摆，叠加咖啡/疲惫提示 | 长时间陪伴感，漂亮但低干扰 | `还在跑...` |
| `waiting_user` | 等用户输入、确认或继续 | 明显抬手挥手，小气泡提醒 | 明显提醒用户回来，但保持美观 | `等你回复` |
| `success` | 任务成功、测试通过 | 抬身展开、星光庆祝 | 短暂庆祝 | `通过啦` |
| `error` | 命令失败、测试失败、异常 | 歪头收缩，叠加放大镜/日志提示 | 有问题但不吓人 | `这里好像出错了` |
...
| 状态 | 播放方式 | 建议帧间隔 | 循环策略 |
| --- | --- | --- | --- |
| `idle` | 站姿呼吸轻摆 | `1.20s` | 独立 `idle` clip，低功耗慢循环 |
| `codex_running` | 跳舞循环 | `0.42s` | 连续循环，动作幅度中等 |
| `command_running` | 终端前倾专注 | `0.38s` | 独立 `command-running` clip，叠加终端光屏 |
| `thinking` | 慢速侧身思考 | `0.72s` | 独立 `thinking` clip，叠加思考光点 |
| `long_running` | 极慢速疲惫陪伴 | `1.00s` | 独立 `long-running` clip，舞台低频梦幻光点 |
| `waiting_user` | 抬手挥手提醒 | `0.70s` | 独立 `waiting-user` clip，叠加气泡提醒 |
| `success` | 星光展开庆祝 | `0.30s` | 独立 `success` clip，短暂星光强调 |
| `error` | 歪头日志提示 | `0.72s` | 独立 `error` clip，慢速错误提示 |
...
v9 决策：用户确认 `codex-running` 的梦幻跳舞效果整体最协调，但“所有状态都像同一个跳舞动作”仍然不够清楚。后续视觉检查发现，基于半透明少女帧做头部、手臂、裙摆的局部拼接会产生残影或切块。因此当前方案以 `codex-running` 作为角色源帧和运行态主循环，其它状态由 `scripts/generate-state-action-frames.py` 生成独立动作目录；每一帧只选择一个完整源姿态，再做轻量节奏变换，避免局部叠加残影。

| 状态 clip | 姿态族 | 动作语义 |
| --- | --- | --- |
| `idle` | `idle-breathing-pose` | 极慢呼吸轻摆，安静待机 |
| `codex-running` | `dance-loop` | 最协调的梦幻跳舞主循环 |
| `command-running` | `terminal-forward-focus` | 身体前倾、上半身更主动，表达正在执行 |
| `thinking` | `thinking-turn-pose` | 侧身转头、托腮感，表达分析思考 |
| `long-running` | `tired-slow-pose` | 低头、下沉、慢速，表达任务跑很久 |
| `waiting-user` | `raised-hand-wave` | 抬手挥手，提醒用户回来 |
| `success` | `star-celebration-pose` | 抬身展开、裙摆扩大，配合星光庆祝 |
| `error` | `concerned-tilt-pose` | 歪头收缩、谨慎观察，配合日志/放大镜提示 |

验收时重点看三件事：人物动作必须协调、像人物自己在动；不同状态的轮廓和姿态必须能区分；单帧里不能出现重复手臂、重复头部、黑色切块或局部拼接断层。
...
4. `codex_running` 使用当前梦幻跳舞动作；其它状态使用从该源帧选择出的独立完整姿态 clip，不再出现小键盘、低协调旧动作或局部拼接残影。
5. 舞台地面必须由 AppKit 动态绘制：水面光带、淡倒影、动态裙摆光点。
6. 8 个状态保留动作目录和元数据，原生人物 clip 按状态独立加载；状态差异由完整源姿态序列、装饰层、气泡和播放频率共同完成。
...
- 角色方向：梦幻白色半透明少女。
- 动作方案：`codex-running` 加载协调跳舞主循环，其它状态加载 `public/assets/dancer-actions/<state>/frame-*.png` 下的独立完整姿态 clip，状态装饰由 AppKit 动态绘制。
...
- 历史测试曾覆盖：所有状态映射到协调跳舞 clip，且全局图片运动保持微幅。该策略已被 v9 取代。

v9 已补充独立完整姿态动作策略：

- 根据最新反馈，统一跳舞主循环导致状态区别仍不明显。因此恢复按状态加载独立 clip。
- 所有非 `codex-running` clip 都从协调跳舞源帧选择完整姿态序列，禁止把多个半透明局部姿态叠在同一帧里。
- 当前测试要求：原生映射必须按状态返回独立 action id；非 `codex-running` clip 的 `action.json` 必须声明 `motionTechnique = single-source-pose-selection`，并带有 `single-source-frame-no-overlay-afterimage` 残影控制；全局位移和旋转仍必须保持低幅度。
